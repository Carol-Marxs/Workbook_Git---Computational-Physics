...
class Particula:
    ...
    def plotar_velocidade_tempo(self):
        plt.figure(figsize=(8, 5))
        plt.plot(self.tempo, self.vx, label='Velocidade X')
        plt.plot(self.tempo, self.vy, label='Velocidade Y')
        plt.xlabel('Tempo (s)')
        plt.ylabel('Velocidade (m/s)')
        plt.title('Velocidades em função do tempo')
        plt.grid(True)
        plt.legend()
        plt.tight_layout()
        plt.show()

    def plotar_posicao_tempo(self):
        plt.figure(figsize=(8, 5))
        plt.plot(self.tempo, self.x, label='Posição X')
        plt.plot(self.tempo, self.y, label='Posição Y')
        plt.xlabel('Tempo (s)')
        plt.ylabel('Posição (m)')
        plt.title('Posições em função do tempo')
        plt.grid(True)
        plt.legend()
        plt.tight_layout()
        plt.show()
...
p.plotar_trajetoria()
p.plotar_velocidade_tempo()
p.plotar_posicao_tempo()
